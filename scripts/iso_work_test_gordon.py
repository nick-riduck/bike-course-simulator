...
def run_iso_work_test(gpx_path):
    ...
    # 2. Gordon's Theory 모델 실행 (이분 탐색으로 에너지 일치)
    print(f"\n[Step 2] Matching Gordon's Theory Engine to {target_work:.1f} kJ...")
    engine_gordon = GordonTheoryEngine(rider, params)
    res_gordon = engine_gordon.find_pbase_for_work(segments, target_work)
    
    print(f"Time: {int(res_gordon.total_time_sec//3600)}h {int((res_gordon.total_time_sec%3600)//60)}m {int(res_gordon.total_time_sec%60)}s")
    print(f"Work: {res_gordon.work_kj:.1f} kJ, NP: {res_gordon.normalized_power:.1f}W")

    # 3. 결과 비교
    diff_sec = res_gordon.total_time_sec - res_asym_ref.total_time_sec
    print(f"\n[Conclusion]")
    if diff_sec > 0:
        print(f"Asymmetric is FASTER than Gordon's Theory by {abs(diff_sec):.1f} seconds.")
    else:
        print(f"Gordon's Theory is FASTER than Asymmetric by {abs(diff_sec):.1f} seconds.")
